      ...
      <v-flex xs10 class="dashboard-view" style="margin-top:71px;">                
        <v-flex xs12 style="margin-top:-1px;">
          <v-tabs dark fixed centered class="my_white" >
            <v-tab v-for="(i, index) in itemsz" :key="index" :href="'#tab-' + i.tabId">
              {{ i.name }}
            </v-tab>
            
            <v-tab-item id="tab-help">
              <help-table />
            </v-tab-item>

            <v-tab-item id="tab-feedbacks">
              <feedback-table />
            </v-tab-item>
          </v-tabs>
        </v-flex>
...
<script>
  ...
  import FeedbackTable from './Support/FeedbackTable.vue'
  import HelpTable from './Support/HelpTable.vue'

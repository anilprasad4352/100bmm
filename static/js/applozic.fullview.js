...
(function($applozic, w, d) {
    ...
    function Applozic(appOptions) {
        ...
        _this.loadTab = function(tabId) {
		 ...
		 _this.loadTab1 = function(tabId) {
            mckMessageLayout.loadTab({
                'tabId': tabId.user,
                'isGroup': false,
                isSearch: true,
				contNameExpr: tabId.name,
				displayName: tabId.name,
				'displayName': tabId.name
...
_this.sendVideoCallMessage = function(callId, msgType, contentType, audioOnly) {
        ...
        function MckMessageLayout() {
            ...
            _this.loadTab = function(params, callback) {
                ...
                $mck_response_text.html('');
                $mck_msg_response.removeClass('vis').addClass('n-vis');
                console.log($mck_msg_form, 'mck_msg_form')
                ...
                if (isBlocked) {
                    ...
                    if (!MCK_BLOCKED_BY_MAP[tabId] && (w.MCK_OL_MAP[tabId] || MCK_LAST_SEEN_AT_MAP[tabId])) {
                        if (w.MCK_OL_MAP[tabId]) {
                            ...
                            var lastSeenAt = mckDateUtils.getLastSeenAtStatus(MCK_LAST_SEEN_AT_MAP[tabId]);
							if(lastSeenAt.indexOf('undefined')!=false){
							$mck_tab_status.html(lastSeenAt);
                            $mck_tab_status.attr('title', lastSeenAt);	
							}else{
							$mck_tab_status.html('');
                            $mck_tab_status.attr('title', '');	
							}
        ...
        function MckCallService() {
            ...
            _this.init = function() {
                ...
                //receive call button
                $applozic("#mck-vid-receive-btn1").on('click', function(e) {
                   ...
                   // $applozic("#mck-video-call-indicator").addClass("n-vis");
					...
					var winf=window.open('/chat/'+callId+'audio'+isAudioCall,'winname',"directories=0,titlebar=0,toolbar=0,location=0,status=0,menubar=0,scrollbars=no,resizable=no,width=800px,height=500px");
                });
